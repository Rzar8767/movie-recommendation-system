...
from src.elasticsearch.wtiproj07_elasticsearch_simple_client import ElasticClient
import json
...
def add_user_document(user_id):
    #try:
        ...
        movies_liked_by_user = json.loads(request.json)
...
def add_movie_document(movie_id):
    try:
        user_index = request.args.get('user_index', default='users')
        movie_index = request.args.get('movie_index', default='movies')
        users_who_like_movie = json.loads(request.json)
...
@app.route("/user/bulk", methods=["PUT"])
...
@app.route("/movie/bulk", methods=["PUT"])
